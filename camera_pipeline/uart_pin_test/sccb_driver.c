...
#define SCCB_GPIO_DEVICE_ID    XPAR_XGPIO_0_BASEADDR
#define SIOC_BIT 0x1
#define SIOD_BIT 0x2
...
void sccb_init(void) {
    int status = XGpio_Initialize(&sccb_gpio, SCCB_GPIO_DEVICE_ID);
    if (status != XST_SUCCESS) {
        xil_printf("ERROR: SCCB GPIO init failed, status=%d\r\n", status);
    }
